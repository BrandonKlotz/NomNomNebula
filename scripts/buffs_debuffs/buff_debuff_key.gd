class_name BuffDebuffKey

const EXTRA_DASHES: String = "extra_dashes"
const DASH_FORCE_FACTOR: String = "dash_force_factor"
const ESCAPING_TIME: String = "escaping_time"
const DASH_RECHARGE_FACTOR: String = "dash_recharge_factor"
const DASH_RECHARGE_FACTOR_INCREASED: String = "dash_recharge_factor_increased"
const MOVEMENT_WARP_FACTOR: String = "movement_warp_factor"
const INTERACTION_RADIUS_FACTOR: String = "interaction_radius_factor"
const ABSORPTION_SPEED_FACTOR: String = "absorption_speed_factor"
const STABILITY_MAX: String = "stability_max"
const CONTROL_TYPE_TANK: String = "control_type_tank"
const CONTROL_TYPE_INVERTED: String = "control_type_inverted"
const LIMIT_DASH: String = "limit_dash"

const DESCRIPTIONS: Dictionary[String, String] = {
	EXTRA_DASHES: "Gives extra bursts",
	DASH_FORCE_FACTOR: "Increases burst speed",
	ESCAPING_TIME: "Increase time to scape black holes",
	DASH_RECHARGE_FACTOR: "Bursts recharge faster",
	DASH_RECHARGE_FACTOR_INCREASED: "Bursts recharge slower",
	MOVEMENT_WARP_FACTOR: "Warp speed incremented",
	INTERACTION_RADIUS_FACTOR: "Galaxies have more radius",
	ABSORPTION_SPEED_FACTOR: "Time to absorbe galaxies increased",
	STABILITY_MAX: "Stability max decreased",
	CONTROL_TYPE_TANK: "Tank movement",
	CONTROL_TYPE_INVERTED: "Movement is inverted",
	LIMIT_DASH: "Only one burst"
}
